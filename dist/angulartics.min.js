...
(h instanceof RegExp&&h.test(f[i])||f[i].indexOf(h)>-1)&&(f[i]="FILTERED");return f.join("/")}return a}function l(a,b,c){f(a)||(a=i(a),a=j(a),a=k(a),d.pageTrack(a,b,c))}var m,n,o;if(d.settings.pageTracking.autoTrackFirstPage){if(/* Only track the 'first page' if there are no routes or states on the page */
...
d.settings.pageTracking.basePath=c.location.pathname+"#"),m=!0,d.settings.pageTracking.trackRoutes&&e.has("$route")){if(n=e.get("$route"))for(o in n.routes){m=!1;break}else null===n&&(m=!1);b.$on("$routeChangeSuccess",function(a,b){b&&(b.$$route||b).redirectTo||e.invoke(["$location",function(a){var b=d.settings.pageTracking.basePath+a.url();l(b,a)}])})}d.settings.pageTracking.trackStates&&(e.has("$state")&&!e.has("$transitions")&&(m=!1,b.$on("$stateChangeSuccess",function(a,b){e.invoke(["$location","$state",function(a,b){var c=d.settings.pageTracking.basePath+a.url();l(c,a,b.$current.locals.globals.resolved_data)}])})),e.has("$state")&&e.has("$transitions")&&(m=!1,e.invoke(["$transitions",function(a){a.onSuccess({},function(a){var b=a.options();
